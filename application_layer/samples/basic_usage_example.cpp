...
int main() {
    ...
    logger->log(kcenon::logger::log_level::info, "Messaging System Basic Usage Example");
    logger->log(kcenon::logger::log_level::info, "=====================================");

    try {
        // 1. Create and configure the messaging system
        logger->log(kcenon::logger::log_level::info, "1. Creating messaging system...");
        ...
        logger->log(kcenon::logger::log_level::info, "2. Initializing system...");
        if (!integrator->initialize()) {
            logger->log(kcenon::logger::log_level::error, "Failed to initialize messaging system!");
            logger->stop();
            return 1;
        }
        ...
        logger->log(kcenon::logger::log_level::info, "3. Setting up message subscribers...");

        // Simple message handler
        integrator->subscribe("user.login", [logger](const message& msg) {
            logger->log(kcenon::logger::log_level::info, "   [Login Handler] User logged in!");
        ...
        integrator->subscribe("order.created", [logger](const message& msg) {
            logger->log(kcenon::logger::log_level::info, "   [Order Handler] New order received!");
        ...
        // 4. Publish messages
        logger->log(kcenon::logger::log_level::info, "4. Publishing messages...");
        ...
        // User login message
        {
            ...
            logger->log(kcenon::logger::log_level::debug, "   Published user login message");
        }
        ...
        // Order creation message
        {
            ...
            logger->log(kcenon::logger::log_level::debug, "   Published order creation message");
        }
        ...
        // Notification messages
        {
            ...
            logger->log(kcenon::logger::log_level::debug, "   Published email notification");
        }

        {
            ...
            logger->log(kcenon::logger::log_level::debug, "   Published SMS notification");
        }

        logger->log(kcenon::logger::log_level::info, "   All messages published!");

        // 5. Wait for message processing
        logger->log(kcenon::logger::log_level::info, "5. Processing messages...");
        std::this_thread::sleep_for(std::chrono::milliseconds(500));
        logger->log(kcenon::logger::log_level::info, "   Message processing complete!");

        // 6. Check system health
        logger->log(kcenon::logger::log_level::info, "6. Checking system health...");
        auto health = integrator->check_system_health();

        logger->log(kcenon::logger::log_level::info, "   System Health Report:");
        logger->log(kcenon::logger::log_level::info,
        ...
        logger->log(kcenon::logger::log_level::info, "7. Accessing services through container...");
        ...
        for (const auto& service_name : registered_services) {
            logger->log(kcenon::logger::log_level::info, "   - " + service_name);
        }

        // 8. Configuration access
        logger->log(kcenon::logger::log_level::info, "8. System configuration:");
        const auto& sys_config = integrator->get_config();
        logger->log(kcenon::logger::log_level::info, "   - Environment: " + sys_config.environment);
        logger->log(kcenon::logger::log_level::info, "   - System name: " + sys_config.system_name);
        logger->log(kcenon::logger::log_level::info, "   - Version: " + sys_config.version);
        logger->log(kcenon::logger::log_level::info,
        ...
        // 9. Shutdown
        logger->log(kcenon::logger::log_level::info, "9. Shutting down system...");
        integrator->shutdown();
        logger->log(kcenon::logger::log_level::info, "   System shutdown complete!");

        logger->log(kcenon::logger::log_level::info, "Example completed successfully!");
        logger->flush();
        logger->stop();

    } catch (const std::exception& e) {
        if (logger) {
            logger->log(kcenon::logger::log_level::error, "Error: " + std::string(e.what()));
            logger->stop();
        }
        return 1;
    }
