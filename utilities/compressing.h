...
namespace compressing
{
	...
	public:
		static bool compression_folder(const wstring& target_file, const wstring& root_path, 
			...
			const wstring& file_header = L"CF100", const function<void(vector<uint8_t>&, const wstring&, const vector<uint8_t>&)>& combination_rule = nullptr);
		static bool decompression_folder(const wstring& source_path, const wstring& target_path, const unsigned short& block_bytes = 1024, 
			const wstring& file_header =L"CF100", const function<void(const vector<uint8_t>&, wstring&, vector<uint8_t>&)>& combination_rule = nullptr);
