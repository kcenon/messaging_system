...
messaging_system:
  ...
  thread_pools:
    ...
    auto io_executor = std::make_shared<kcenon::thread::thread_pool>(config.thread_pools.io_workers);
    auto work_executor = std::make_shared<kcenon::thread::thread_pool>(config.thread_pools.work_workers);
    auto router = std::make_shared<TopicRouter>(work_executor);
    auto message_bus = std::make_shared<MessageBus>(io_executor, work_executor, router);
